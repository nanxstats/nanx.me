...
```{mermaid, echo=FALSE, eval=FALSE}
graph LR
	A(Domain <br> Nameserver) -->
  |point to| B("Cloudflare" )
	B --> |verify cert| C(Nginx)
  C -->|reverse proxy| D(Shiny Server)
	D --> E(App 1)
	D --> F(App 2)
	D --> G(App 3)
```

![A diagram of the checkpoints. Made with the [Mermaid live editor](https://mermaid-js.github.io/mermaid-live-editor/).](/image/self-hosting-shiny-diagram.svg)
...
- [How to set up Cloudflare with Linode](https://www.linode.com/docs/networking/dns/how-to-set-up-cloudflare-with-linode/)
